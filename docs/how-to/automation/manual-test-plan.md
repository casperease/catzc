...
| 58  | L2   | automation/Catzc.Base.Globs/tests/PipelineTrigger-Integrity.Tests.ps1                      | Native trigger globs match the projection                            | Every pipeline-bound globset's ADO pipeline (and the GitHub CI workflow) declares exactly the trigger path filters its native projection computes — a drifted `paths:` filter fails.                           |
