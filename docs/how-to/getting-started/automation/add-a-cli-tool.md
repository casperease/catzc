...
`Assert-Tool`, `Get-ToolConfig`, `Get-ToolInstallOrder`) reads the config and picks the mechanism:

- **Native package managers** — winget on Windows, brew on macOS, apt on Linux — for OS-level tools (`winget_id` / `brew_formula` /
  `apt_package`), all user-scope.
- **uv** for the Python family (`uv_python` provisions Python itself; `uv_tool` installs a standalone Python CLI like the Azure CLI or
...
1. **Add the entry** to `tools.yml` with `version`, `command`, `version_command`, `version_pattern`, and the install keys for its mechanism
   (`winget_id` / `brew_formula` / `apt_package`; `uv_tool` for a standalone Python CLI; `pip_package` for an importable library;
   `uv_python: true` for Python itself; or `script_install: true` with `windows_install_dir` / `unix_install_dir`). Add `depends_on: <tool>`
   for install ordering (`depends_on: uv` for anything uv installs) — `Get-ToolInstallOrder` topologically sorts installs.
2. **Add the triad** in the matching ecosystem module — `Catzc.Tooling.Python` for uv-pip library tools, `Catzc.Tooling.Node` for npm tools,
   `Catzc.Tooling.Toolchain` for native-package-manager / uv-tool / script tools (one function per file), each delegating to the engine:
   - `Install-<Tool>` — installs the locked version, idempotent, `-Force` replaces a wrong version. Delegate to `Install-UvTool` for a
     `uv_tool`, `Install-PipTool` for a `pip_package` library, or `Install-Tool` for a native-package-manager tool.
