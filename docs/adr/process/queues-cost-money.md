...
CI/CD, using the two laws from [ADR-LEAN](lean.md) — Little's Law for how queues set lead time, and Moore's Law for why the cheapest way to
drain a queue is usually to spend machine time.
...
run anywhere ([ADR-PARITY](../automation/devbox-pipeline-parity.md)) is what lets a developer pay a few seconds of local CPU to avoid a
