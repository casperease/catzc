...
## Rules: ADR-PROC-PULLWORK

### Rule ADR-PROC-PULLWORK:1
...
ready creates queues and partially-done work ([ADR-PRIN-NOWASTE](../principles/reduce-waste.md), the partially-done-work waste).

- [Decision](#decision)

### Rule ADR-PROC-PULLWORK:2
...
keep queues short ([ADR-PROC-QUEUECOST](queues-cost-money.md)); large batches do the opposite on every count.

- [Decision](#decision)

### Rule ADR-PROC-PULLWORK:3
...
the task-switching waste ([ADR-PRIN-NOWASTE](../principles/reduce-waste.md)) — by Little's Law, less work-in-progress is directly less lead
time ([ADR-PROC-QUEUECOST](queues-cost-money.md)).
...
([ADR-PRIN-NOWASTE](../principles/reduce-waste.md)) — and the cost is the same: capital tied up, merge risk accumulating, and the real
bottleneck hidden behind a full queue. This article states the pull discipline; its cost side — why the queues are where the money goes — is
...
- **Small batches.** Break work down to the smallest increment that can be integrated and delivered on its own. This is the same move agile
  makes for responsiveness ([ADR-PROC-VALUES](agile-values.md)) and lean makes for flow — one lever, two payoffs.
- **Defer commitment.** Make irreversible decisions at the last responsible moment, when knowledge is greatest ([ADR-PROC-LEAN](lean.md)).
  This is not procrastination: it is refusing to spend certainty you do not yet have, while still deciding before indecision itself becomes
  the cost.
...
dominant cost in the system ([ADR-PROC-QUEUECOST](queues-cost-money.md)). Pull caps the inventory by construction.
...
a queue as partially-done work ([ADR-PRIN-NOWASTE](../principles/reduce-waste.md)). Break a change into the smallest slice that can
integrate and deliver on its own, rather than batching several concerns into one large push. Hold irreversible decisions until the last
responsible moment, and keep the number of changes in flight small enough that the flow, not the plan, sets the pace.
