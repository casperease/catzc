...
build, or the first line of execution — never at production time and never silently ([ADR-POKA](../principles/poka-yoke.md)).
...
bad ones back. W. Edwards Deming's objection, adopted by the Toyota Production System, is that inspection at the end is both too late and
too weak — it finds defects after the cost of making them is already sunk, and it cannot find them all. Toyota's answer is jidoka: build
quality into each step so the output is correct by construction, and stop the line when it is not [^2].

The Poppendiecks carry this into software as "build integrity in" [^1]. It is the natural partner of poka-yoke
([ADR-POKA](../principles/poka-yoke.md)): where poka-yoke is the mechanism (prevent, else detect early), building quality in is the policy
that there is no other mechanism — no end-of-line QA phase to fall back on.
...
- **Prevent, then detect.** The first choice is to make the defect impossible to express — derive behaviour from convention so there is
  nothing to get wrong ([ADR-POKA](../principles/poka-yoke.md), [ADR-LESSVAR](../principles/reduce-variability.md)). Where that is
...
than routing defects to a rework area ([ADR-HOLDLINE](holding-the-line.md)).
...
([ADR-POKA](../principles/poka-yoke.md)). If not, add the check that catches it earliest and wire it into the gates that run in both
...
[^2]:
    Taiichi Ohno, _Toyota Production System_ (1988), on jidoka; and W. Edwards Deming, _Out of the Crisis_ (1986), "Cease dependence on
    inspection to achieve quality" — the origin of building quality in rather than inspecting it in.
