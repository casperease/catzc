...
not on proxies like tasks-marked-done or hours spent; working software is the measure of progress ([ADR-AGILEP](agile-principles.md)).
...
Lean's response is to make it visible deliberately. This repository already does so in two places: the value-chain diagrams render every
commit's delivery state ([ADR-VISUAL](../design/visual-design.md)), and console output is treated as a first-class channel that reports what
actually happened ([ADR-CONSOLE](../automation/powershell/console-output-matters.md)). This article states the underlying rule those
specialise.
...
- **Go and see.** Judge progress from the real running artifact and real signals — a green gate, a deployed environment, a monitored metric
  — not from a proxy. "Done" means the software does the thing, observed, not a task ticked ([ADR-AGILEP](agile-principles.md), working
...
**You cannot improve what you cannot see.** Inspect-and-adapt ([ADR-ADAPT](inspect-and-adapt.md)) needs evidence, and evidence needs
...
[^1]:
    Taiichi Ohno, _Toyota Production System_ (1988), on genchi genbutsu ("go and see") and the visual workplace — abnormalities are made to
    announce themselves at the place they occur.
