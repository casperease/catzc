...
## Rules: ADR-PROC-OBSERVEWIP

### Rule ADR-PROC-OBSERVEWIP:1
...
inferred from memory or a status meeting ([ADR-DSGN-VISUAL](../design/visual-design.md), [ADR-DSGN-LIFE](../design/commit-lifecycle.md)).

- [Decision](#decision)

### Rule ADR-PROC-OBSERVEWIP:2
...
not on proxies like tasks-marked-done or hours spent; working software is the measure of progress ([ADR-PRIN-PROCESS](agile-principles.md)).

- [Decision](#decision)

### Rule ADR-PROC-OBSERVEWIP:3
...
([ADR-AUTO-CONSOLE](../automation/powershell/console-output-matters.md)); a problem is made visible where it happens, never buried or
deferred.

- [How to apply](#how-to-apply)

### Rule ADR-PROC-OBSERVEWIP:4
...
station while starving the whole (optimise the whole; [ADR-PROC-LEAN](lean.md)).
...
delivery state ([ADR-DSGN-VISUAL](../design/visual-design.md)), and console output is treated as a first-class channel that reports what
actually happened ([ADR-AUTO-CONSOLE](../automation/powershell/console-output-matters.md)). This article states the underlying rule those
specialise.
...
- **Render the flow.** The position and state of every commit is shown by construction, not reconstructed on demand
  ([ADR-DSGN-VISUAL](../design/visual-design.md)): position is time, colour is the furthest state reached, and a glance decodes where a
  change is. The value stream is the visual factory.
- **Go and see.** Judge progress from the real running artifact and real signals — a green gate, a deployed environment, a monitored metric
  — not from a proxy. "Done" means the software does the thing, observed, not a task ticked ([ADR-PRIN-PROCESS](agile-principles.md),
...
- **Announce, don't bury.** When something happens that a human needs to know — a slow step about to block, a problem detected — it is said
  plainly on the console at the moment it happens ([ADR-AUTO-CONSOLE](../automation/powershell/console-output-matters.md)), not swallowed
  and discovered later.

## Why

**You cannot improve what you cannot see.** Inspect-and-adapt ([ADR-PROC-KAIZEN](inspect-and-adapt.md)) needs evidence, and evidence needs
...
commit colour ([ADR-DSGN-VISUAL](../design/visual-design.md)), a console line
([ADR-AUTO-CONSOLE](../automation/powershell/console-output-matters.md)), a monitored signal — and make that the default, not an opt-in.
Judge whether work is done by observing the real artifact, not by a proxy. When you reason about speed, look at the whole value stream and
find where items wait, rather than at how fast any single station runs.
