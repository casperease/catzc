...
## Rules: ADR-DSGN-SELFSERV

### Rule ADR-DSGN-SELFSERV:1
...
(`ADR-DSGN-THINPLAT`), without filing a ticket to a central gatekeeper. This is only defensible because every self-service action is a
...
tooling — all represented as code (`ADR-PRIN-EAC`). Self-service means editing that source and letting the platform apply it; it never means
...
from source to deployed state. The globset durable-SHA identities (`ADR-FLOW-CD-GLOBS`) content-address each shippable unit; a build is a
verifiable path from that commit to its output, not a one-off act that cannot be repeated. This is what makes self-service _auditable_:
every deployed thing traces to one exact, reviewed commit, and anyone can independently rebuild it and confirm the match. (Primary sources:
the Reproducible Builds project on the source-to-binary verification path, and SLSA on build provenance and attestation.)

- [Immutable artifacts and provenance](#immutable-artifacts-and-provenance)

### Rule ADR-DSGN-SELFSERV:4
...
own code (`ADR-PRIN-ONELIVE`). The guardrails — asserts (`ADR-AUTO-FAILFAST`), the quality gates, the freshness/marker checks, and code
review — are **automated and mandatory**, so self-service cannot merge a change that skips them. Speed and control are the same mechanism
here, not a trade-off. (Primary source: trunkbaseddevelopment.com, Paul Hammant.)

- [Trunk-based development is the guardrail](#trunk-based-development-is-the-guardrail)

### Rule ADR-DSGN-SELFSERV:5
...
truth** (`ADR-PRIN-EAC`), **immutable and reproducibly-built artifacts** with provenance, **trunk-based development** with automated
guardrails (`ADR-PRIN-ONELIVE`), and the **thin CLI** as the only way to act (`ADR-DSGN-THINPLAT`). Each one is load-bearing; remove any and
...
comprises (`ADR-FLOW-CD-GLOBS`), so "which version is deployed" has a single, verifiable answer. Reproducibility makes that answer checkable
by a third party — the Reproducible Builds project defines a reproducible build as "an independently-verifiable path from source to binary
code", which is exactly the property an auditor needs. Provenance carries the chain the other direction: SLSA frames build provenance as
verifiable evidence of _how_ an artifact was produced. Together they mean a deployed artifact is not trusted on faith — it is trusted
because it can be traced back to a reviewed commit and rebuilt to match.
...
stale marker. The branching discipline and the no-legacy rule (`ADR-PRIN-ONELIVE`) are two faces of the same commitment: one living version,
on trunk, always releasable.
...
of truth** (ADR-DSGN-SELFSERV:2), applied only through the **thin CLI** (ADR-DSGN-SELFSERV:1), shipped as an **immutable, reproducibly-built
artifact with provenance** (ADR-DSGN-SELFSERV:3), landed via **trunk-based development behind automated, mandatory guardrails**
(ADR-DSGN-SELFSERV:4). In controlled domains this makes self-service and auditability the same mechanism rather than competing ones
(ADR-DSGN-SELFSERV:5).

### How this is enforced

- **Everything-as-code** (`ADR-PRIN-EAC`) keeps the repository the sole authoritative source; there is no supported out-of-band channel.
- **The globset durable-SHA identities** (`ADR-FLOW-CD-GLOBS`) content-address every shippable unit, giving each deployment a single
  verifiable identity.
- **One living version** (`ADR-PRIN-ONELIVE`) forbids long-lived branches and legacy variants, keeping trunk the one always-releasable
  source.
- **Fail-fast asserts and the quality gates** (`ADR-AUTO-FAILFAST`, the spell/terminology/freshness gates) are the automated guardrails a
  self-service change must clear before it can merge.
- **The thin CLI** (`ADR-DSGN-THINPLAT`) is the only actuator, so no self-service action can bypass the record.
