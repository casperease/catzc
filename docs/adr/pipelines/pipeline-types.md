...
- **Trigger:** ADO build validation on the PR branch (pre-commit) and a master trigger (post-commit), path-filtered to the unit it verifies.
- **Example — CI-automation:** "builds" by running `importer.ps1` and "tests" via `Test-Automation` (see
  ...
  green. It triggers on changes to `/automation`.
