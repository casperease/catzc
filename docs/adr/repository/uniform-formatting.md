...
formatter-stable: running the formatter over it changes nothing. A hand-padded string template is the same hand-formatting ADR-FORMAT:1 forbids,
merely moved into code; it drifts the moment a field is added and churns the diff on every build.
