...
path filter of a policy that queues the resolved pipeline on the guarded branch — creating a missing policy, updating a drifted one in place
(matched by pipeline definition + branch, so a marker-path or display-name change never duplicates), and leaving a current one alone
