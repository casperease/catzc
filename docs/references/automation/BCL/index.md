...
loaded at import. Those types compile against the BCL and **nothing else** — there is no NuGet in this toolset, so a type may use only what
the shared framework ships (see [native-csharp-types rule ADR-TYPES:5](../../../adr/automation/BCL/native-csharp-types.md)). This area collects
the BCL-side reference material.
