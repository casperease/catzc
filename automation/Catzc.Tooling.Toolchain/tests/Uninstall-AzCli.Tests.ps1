Describe 'Uninstall-AzCli' -Tag 'L1', 'logic' {
    ...
    It 'escalates to Remove-AzCli -Force after the managed uninstall with -Remove -Force' -Tag 'ADR-REMOVE#5' {
    ...
    It 'escalates as a dry-run — Remove-AzCli without -Force — when -Remove has no -Force' -Tag 'ADR-REMOVE#4' {
        Uninstall-AzCli -Remove
        Should -Invoke Remove-AzCli -ModuleName Catzc.Tooling.Toolchain -Times 1 -ParameterFilter { -not $Force }
    }

    It 'still evicts when the managed uninstall fails — the case -Remove exists for' -Tag 'ADR-REMOVE#5' {
