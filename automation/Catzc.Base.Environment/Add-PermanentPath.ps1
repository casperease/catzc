...
function Add-PermanentPath {
    ...
    # --- Persistent PATH ---
    # Get-EnvironmentPath / Set-EnvironmentPath are the registry seam — kept thin so tests Mock them instead
    # of touching the real machine PATH. See their help.
    if ($IsWindows) {
        ...
        if (-not $alreadyPersisted) {
            ...
            Set-EnvironmentPath ($userEntries -join ';') -Scope User
        }
    }
    else {
        $profilePath = $PROFILE.CurrentUserCurrentHost
        $startMarker = ">>> catzc PATH $Label >>>"
