Describe 'Copy-Object' -Tag 'L0', 'logic' {
    ...
    Context 'null and scalar values' {
        ...
        It 'passes through datetime' {
            $dateTime = [datetime]'2025-06-15T12:30:00'
            Copy-Object $dateTime | Should -Be $dateTime
        }
    ...
    Context 'real-world: nested yaml config asset' {
        BeforeAll {
            $assetPath = Join-Path $PSScriptRoot 'assets\copy-object-config.yml'
            $script:config = Get-Content $assetPath -Raw | ConvertFrom-Yaml -Ordered
