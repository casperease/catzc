...
public sealed class GlobSet
{
    ...
    // The marker file's full content (ADR-GLOBS:9): the definition representation plus a small readable digest
    // and TWO identities — 'files' (the count of git-bound members, a checkable at-a-glance summary), the list
    // identity 'scoped_sha256' (the ordered set of member PATHS, names only — DurableHash.HashPathList,
    // ADR-GLOBS:5), and the durable content identity 'sha256' (the path|content folds). The marker stays lean
    // and fully deterministic: the meta, the scan program, a count, and two SHAs — bound to the committed file
    // NAMES and their content, and NOTHING else (no file list, no local-tree facts). The expanded member list
    // is written separately to the transient out/ folder (Write-CompanionFile, ADR-GLOBS:11), never here.
    // Fixed field order keeps the diff legible: scan = "definition changed", files/scoped_sha256 = "membership
    // changed", sha256 = "content changed".
    public string MarkerContent(int fileCount, string scopedSha256, string sha256)
    {
        if (fileCount < 0)
        {
            throw new ArgumentException(string.Format("globset '{0}': MarkerContent requires a non-negative file count", Name));
        }
        ...
        return Representation + "files: " + fileCount + "\n" + "scoped_sha256: " + scopedSha256 + "\n" + "sha256: " + sha256 + "\n";
    }
