...
// description, its layer (deployable-unit | loose-fileset, ADR-FLOW-CD-GLOBS:7; the derived module layer never
// appears in the registry), the authored include/exclude patterns, and the optional composition
// (ADR-FLOW-CD-GLOBS:8). Membership is decided by the set's SCAN PROGRAM (ADR-FLOW-CD-GLOBS:4): an ordered list of +/- rules
...
// (ADR-FLOW-CD-GLOBS:5) is computed over exactly what the program selects.
// Verify (test blast-radius scope) and Pipeline (the trigger-role binding) are declarative meta annotations.
// A globset never composes itself and the compose graph is acyclic (ADR-FLOW-CD-GLOBS:8), which GlobsConfig asserts
// across the whole registry at construction.
// Per-layer independence (ADR-FLOW-CD-GLOBS:10) is decided on OWN membership (OwnMatches — the set's own rules only,
...
public sealed class GlobSet
{
    ...
    // The layers (ADR-FLOW-CD-GLOBS:7) — three kinds of thing: 'deployable-unit' (a configurable unit that ships) and
    ...
    // overlaps the boundaries it cuts across). 'module' is the DERIVED layer (ADR-REPO-PROTGLOB) — the per-folder
    ...
    // 'loose-fileset' the sets are pairwise-disjoint on OWN membership (ADR-FLOW-CD-GLOBS:10).
    ...
    // A single scan-program rule (ADR-FLOW-CD-GLOBS:4): Select true = '+' (select), false = '-' (drop).
        ...
        string[] compose, string[] verifyModules, int verifyLevel, string pipeline)
    {
        ...
        if (string.IsNullOrWhiteSpace(layer) || Array.IndexOf(ValidLayers, layer) < 0)
        {
            throw new ArgumentException(string.Format(
                "globset '{0}': layer '{1}' is not a layer (allowed: {2}, ADR-FLOW-CD-GLOBS:7)",
    ...
    // Membership (ADR-FLOW-CD-GLOBS:4): evaluate the scan program last-match-wins, default not-selected — a file
    ...
    // OWN membership (ADR-FLOW-CD-GLOBS:10): the set's own program only — its includes as '+' then its excludes as
    ...
    // The flattened scan program (ADR-FLOW-CD-GLOBS:4/8): the composed sets' programs first (dependency order,
