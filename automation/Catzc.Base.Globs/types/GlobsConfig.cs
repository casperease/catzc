...
public sealed class GlobsConfig
{
    ...
    public GlobsConfig(IDictionary raw)
    {
        ...
        foreach (object key in raw.Keys)
        {
            ...
            if (top != "globsets")
            {
                errors.Add(string.Format("unknown top-level key '{0}' (allowed: globsets)", top));
            }
        }
        ...
        // A broken compose graph makes Matches() unsafe (a cycle would recurse forever), so fail now with
        // everything collected so far — the final validation gate.
        if (errors.Count > 0)
        {
