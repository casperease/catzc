<#
.SYNOPSIS
    Reports per-layer independence violations (ADR-FLOW-CD-GLOBS:10): same-layer globsets that select a common
...
.DESCRIPTION
    ...
    reserved umbrellas) and overlap by design (ADR-FLOW-CD-GLOBS:7).

    Evaluates the declared registry (Get-GlobSet) AND the derived module sets (Get-ModuleGlobSet,
    ADR-REPO-PROTGLOB:7) against the tracked-file universe, and returns one object per violating pair — Layer, A,
...
function Test-GlobSetIndependence {
    ...
    # loose-filesets are cross-cutting and overlap-exempt (ADR-FLOW-CD-GLOBS:7/10); every other layer is checked.
    $exemptLayer = 'loose-fileset'

    $sets = @(Get-GlobSet) + @(Get-ModuleGlobSet)
    $tracked = @(Get-TrackedFile)

    # OWN members per set (compose ignored, ADR-FLOW-CD-GLOBS:10), computed once as a set for O(1) intersection.
    $own = @{}
    foreach ($set in $sets) {
