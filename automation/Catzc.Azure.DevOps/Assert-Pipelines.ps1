<#
.SYNOPSIS
    Asserts the pipelines/ tree obeys the naming-and-placement contract (ADR-PIPENAME); throws on any
...
.DESCRIPTION
    ...
    See Test-Pipelines for the rules enforced (ADR-PIPENAME:1/2/3/4/6) and docs/adr/pipelines/
...
function Assert-Pipelines {
    ...
    if ($violations.Count -eq 0) {
        Write-Message 'All pipelines satisfy the naming-and-placement contract (ADR-PIPENAME).'
        return
    }

    $detail = ($violations | ForEach-Object { "  [$($_.Rule)] $($_.File): $($_.Message)" }) -join "`n"
    throw "$($violations.Count) pipeline naming/placement violation(s) found (ADR-PIPENAME):`n$detail"
}
