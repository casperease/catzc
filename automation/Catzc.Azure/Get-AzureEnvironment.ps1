...
.DESCRIPTION
    ...
    (via Get-AzureSubscription). The subscription is named directly — a template targets a subscription by
    its config folder, and the deploy resolves which one — so it is an explicit input here. Asserts the
    subscription actually serves the environment. The embedded subscription's optional `customer` is what
    renders into resource names. See docs/adr/azure/data-model.md#rule-adr-datamod4.
