<#
.SYNOPSIS
    Returns the per-subscription identity environment (subn/subp) of a named subscription.
.DESCRIPTION
    Each subscription carries exactly one per-subscription env (subn or subp — see
...
.EXAMPLE
    Get-AzureSubscriptionEnvironment shared_nonprod   # -> subn
.EXAMPLE
    Get-AzureSubscriptionEnvironment apex_prod         # -> subp
#>
function Get-AzureSubscriptionEnvironment {
    ...
    Assert-True ($perSubscriptionEnvironments.Count -ge 1) -ErrorText "Subscription '$Subscription' has no per-subscription environment (subn/subp)."
