Describe 'Write-TestAutomationRuleCoverage' -Tag 'L0', 'logic' {
    BeforeAll {
        ...
        $script:allRuleIds = @('ADR-ERROR#3', 'ADR-ERROR#5', 'ADR-NOPWD#1', 'ADR-IDEM#1', 'ADR-LONELY#9')
        $script:rows = @(
            [pscustomobject]@{ ExpandedPath = 'M.a cites error'; Rules = 'ADR-ERROR#3' }
            [pscustomobject]@{ ExpandedPath = 'M.b cites error and idem'; Rules = 'ADR-ERROR#3;ADR-IDEM#1' }
            [pscustomobject]@{ ExpandedPath = 'M.c cites nothing'; Rules = '' }
        )
        $script:analyzer = @(
            [pscustomobject]@{ AdrId = 'ADR-NOPWD#1'; Enforcer = 'Measure-NeverDependOnPwd'; Kind = 'pssa-rule' }
            [pscustomobject]@{ AdrId = 'ADR-ERROR#5'; Enforcer = 'Measure-NoWriteErrorOrWarning'; Kind = 'pssa-rule' }
        )
    ...
    It 'lists the rule enforced by neither a test nor an analyzer as uncovered' {
        ...
        $script:md | Should -Match '- ADR-LONELY#9'
    }

    It 'counts every tagged test that cites a rule as an enforcer' {
        $script:md | Should -Match '\| ADR-ERROR#3 \| 2 \|'
        # the ADR-ERROR#3 pester-test enforcers are both cited rows
        @($script:csv | Where-Object { $_.AdrId -eq 'ADR-ERROR#3' -and $_.Kind -eq 'pester-test' }).Count | Should -Be 2
    }

    It 'credits an analyzer rule as a pssa-rule enforcer' {
        $script:md | Should -Match 'Measure-NeverDependOnPwd'
        ($script:csv | Where-Object { $_.AdrId -eq 'ADR-NOPWD#1' }).Kind | Should -Be 'pssa-rule'
    }

    It 'writes an uncovered csv row for a rule with no enforcer' {
        ($script:csv | Where-Object { $_.AdrId -eq 'ADR-LONELY#9' }).Kind | Should -Be 'uncovered'
    }
}
