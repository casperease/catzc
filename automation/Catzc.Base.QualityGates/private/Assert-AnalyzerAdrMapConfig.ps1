...
.DESCRIPTION
    ...
    the .scriptanalyzer folder (the ADR-AZ-CUSTOMER:3 pattern — keep config load hermetic).
...
function Assert-AnalyzerAdrMapConfig {
    ...
    foreach ($rule in $ruleNames) {
        ...
        foreach ($id in $ids) {
            if ($id -cnotmatch '^ADR-[A-Z]+(?:-[A-Z]+)*#\d+$') {
                $errors.Add("analyzer '$rule' has a malformed citation '$id' (want ADR-<CODE>#<n>, e.g. ADR-AUTO-ERROR#3)")
            }
        }
    }
