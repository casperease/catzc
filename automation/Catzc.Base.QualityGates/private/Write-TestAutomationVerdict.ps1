...
.DESCRIPTION
    ...
    gradient (green-rainbow on a passing run, red-rainbow on a failing one, ADR-AUTO-CONSOLE:7) while its titled line
...
.PARAMETER Result
    ...
    or 'Failed' is rejected at binding (fail-fast, ADR-AUTO-FAILFAST). It selects the gradient anchor: green for a
    pass, red for a fail.
.PARAMETER Summary
...
function Write-TestAutomationVerdict {
    ...
    # verdict and nowhere else (ADR-AUTO-CONSOLE:7 "sparingly"). The rule lines take the per-character walk; the
    ...
    # Invariant so the duration reads the same on any devbox culture (ADR-AUTO-XPLAT:6), matching the title's summary.
