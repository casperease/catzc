Describe 'Get-ToolConfig' -Tag 'L0' {
    ...
    Context 'structural validation' -Tag 'integrity' {
        ...
        It '<Tool> has at least one install mechanism' -ForEach $script:toolEntries {
            $hasMechanism = $Config['winget_id'] -or $Config['brew_formula'] -or $Config['apt_package'] -or $Config['pip_package'] -or $Config['script_install'] -or $Config['npm_package']
            $hasMechanism | Should -BeTrue -Because "$Tool needs WingetId, BrewFormula, AptPackage, PipPackage, ScriptInstall, or NpmPackage"
        }
