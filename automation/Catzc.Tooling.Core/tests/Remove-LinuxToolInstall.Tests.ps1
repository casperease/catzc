Describe 'Remove-LinuxToolInstall' -Tag 'L1', 'logic' {
    BeforeEach {
        # A neutral fixture tool (ADR-TEST:3) — never a real tool identity.
    ...
    It 'removes an apt-owned shadow via its owning package and asserts root' -Tag 'ADR-REMOVE#6', 'ADR-REMOVE#7' {
    ...
    It 'refuses the apt path when not elevated — apt needs root' -Tag 'ADR-REMOVE#6' {
        Mock Test-Command { $true } -ParameterFilter { $Command -eq 'dpkg' } -ModuleName Catzc.Tooling.Core
        Mock Invoke-Executable { [pscustomobject]@{ ExitCode = 0; Output = 'gadget-cli: /usr/bin/widget' } } -ParameterFilter { $Command -like 'dpkg -S*' } -ModuleName Catzc.Tooling.Core
    ...
    It 'removes a uv-Python pip shadow user-space, without asserting root' -Tag 'ADR-REMOVE#6', 'ADR-REMOVE#7' {
    ...
    It 'apt ownership wins over a uv-pip shadow (precedence)' -Tag 'ADR-REMOVE#7' {
        Mock Test-Command { $true } -ParameterFilter { $Command -eq 'dpkg' } -ModuleName Catzc.Tooling.Core
        Mock Test-Command { $true } -ParameterFilter { $Command -eq 'uv' } -ModuleName Catzc.Tooling.Core
        Mock Invoke-Executable { [pscustomobject]@{ ExitCode = 0; Output = 'gadget-cli: /usr/bin/widget' } } -ParameterFilter { $Command -like 'dpkg -S*' } -ModuleName Catzc.Tooling.Core
        Mock Invoke-Executable { [pscustomobject]@{ ExitCode = 0; Output = 'Name: widgetlib' } } -ParameterFilter { $Command -like 'uv pip show*' } -ModuleName Catzc.Tooling.Core
    ...
    It 'deletes a stray on-PATH binary no manager owns, user-space' -Tag 'ADR-REMOVE#6', 'ADR-REMOVE#7' {
    ...
    It 'returns false and touches nothing when the tool is not on PATH' -Tag 'ADR-REMOVE#3' {
