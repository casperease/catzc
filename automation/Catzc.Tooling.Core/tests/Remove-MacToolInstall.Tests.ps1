Describe 'Remove-MacToolInstall' -Tag 'L1', 'logic' {
    ...
    It 'uninstalls a Homebrew-owned shadow by its formula, user-space' -Tag 'ADR-REMOVE#6', 'ADR-REMOVE#7' {
    ...
    It 'falls through to the uv-pip shadow when brew does not own the binary' -Tag 'ADR-REMOVE#7' {
        Mock Test-Command { $true } -ParameterFilter { $Command -eq 'brew' } -ModuleName Catzc.Tooling.Core
    ...
    It 'deletes a stray binary when no manager owns it, user-space' -Tag 'ADR-REMOVE#6' {
