Describe 'Assert-ToolVersion' -Tag 'L0', 'logic' {
    ...
    BeforeEach {
        ...
        Mock Get-Command { [pscustomobject]@{ Source = 'C:\az' } } -ModuleName Catzc.Tooling.Core
    ...
    Context 'devbox session (not a pipeline)' {
        ...
        It 'accepts an installed version matching devbox_version even when off the locked pin' {
            Mock Get-ToolConfig { [pscustomobject]@{ version = '2.87'; command = 'az' } } -ModuleName Catzc.Tooling.Core
            Mock Get-ToolVersion { '2.84.0' } -ModuleName Catzc.Tooling.Core
            { & (Get-Module Catzc.Tooling.Core) { Assert-ToolVersion -Tool 'az_cli' } } | Should -Not -Throw
        }

        It 'still enforces the locked version for a tool with no devbox_version lever' {
            Mock Get-ToolConfig { [pscustomobject]@{ version = '3.14'; command = 'python' } } -ModuleName Catzc.Tooling.Core
            Mock Get-ToolVersion { '3.11.0' } -ModuleName Catzc.Tooling.Core
            { & (Get-Module Catzc.Tooling.Core) { Assert-ToolVersion -Tool 'python' } } | Should -Throw '*version mismatch*'
        }
    }
    ...
    Context 'pipeline session (deterministically locked)' {
        ...
        It 'ignores devbox_version and rejects an off-pin version' {
            Mock Get-ToolConfig { [pscustomobject]@{ version = '2.87'; command = 'az' } } -ModuleName Catzc.Tooling.Core
            Mock Get-ToolVersion { '2.84.0' } -ModuleName Catzc.Tooling.Core
            { & (Get-Module Catzc.Tooling.Core) { Assert-ToolVersion -Tool 'az_cli' } } | Should -Throw '*version mismatch*'
        }
